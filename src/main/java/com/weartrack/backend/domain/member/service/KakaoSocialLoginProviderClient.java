...
/**
 * Kakao 소셜 로그인 연동을 처리한다.
 */
@Component
public class KakaoSocialLoginProviderClient implements SocialLoginProviderClient {
    ...
    public KakaoSocialLoginProviderClient(
    ...
    /**
     * Kakao 인가 코드를 access token으로 교환한 뒤 사용자 정보를 조회합니다.
     */
    @Override
    public SocialUserInfo getUserInfo(String authorizationCode, String state) {
        try {
            ...
            JsonNode kakaoAccount = body == null ? null : body.path("kakao_account");

            return new SocialUserInfo(
                    supports(),
                    getRequiredText(body, "id"),
                    getOptionalText(kakaoAccount, "email")
            );
    ...
    /**
     * Kakao token endpoint와 통신해 authorization code를 access token으로 교환합니다.
     */
